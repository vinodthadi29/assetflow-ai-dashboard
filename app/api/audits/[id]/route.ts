...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withAuth(async (req, auth) => {
    ...
    if (v.status === 'VERIFIED') { updates.verified_at = new Date().toISOString(); updates.verified_by = auth.userId }

    const { data, error } = await supabase.from('audit_records').update(updates).eq('id', id).select(`*, asset:assets(id, asset_id, name)`).single()
