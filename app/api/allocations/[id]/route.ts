...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withRoleAuth(async (req, auth) => {
    ...
    const { data: allocation } = await supabase.from('allocations').select('*').eq('id', id).maybeSingle()
    if (!allocation) return NextResponse.json({ error: 'Not found' }, { status: 404 })

    const updates: any = { status: v.status, updated_at: new Date().toISOString() }
    ...
    const { data, error } = await supabase.from('allocations').update(updates).eq('id', id).select(`*, asset:assets(id, asset_id, name), toUser:users!allocations_to_user_id_fkey(id, name)`).single()
    ...
    await logAuditActivity({ userId: auth.userId, action: 'ALLOCATION_UPDATED', entityId: id, description: `Status changed to ${v.status}` })
