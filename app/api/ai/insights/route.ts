...
export async function GET(request: NextRequest) {
  return withAuth(async (_req, _auth) => {
    ...
    const idleAssets = assets.filter(a =>
      new Date(a.created_at) < ninetyDaysAgo && !allocations.find(al => al.asset_id === a.id)
    )

    if (idleAssets.length > 0) {
      insights.push({
        type: 'warning',
        title: `${idleAssets.length} Assets Idle for 90+ Days`,
    ...
    if (maintenance.length > 0) {
      insights.push({
        ...
        confidence: 0.98,
        actionItems: [{ action: 'Schedule maintenance batch', impact: 'Prevent asset failures' }],
      })
    }
    ...
    if (utilizationRate < 60 && assets.length > 5) {
      ...
      insights.push({
        type: 'opportunity',
        title: `Potential Savings: $${Math.round(savings / 1000)}K`,
        description: `Asset utilization is ${Math.round(utilizationRate)}%. Optimizing allocation could reduce costs.`,
        impact: 'high',
        confidence: 0.85,
        actionItems: [{ action: 'Review under-utilized assets', impact: `Save up to $${Math.round(savings)}` }],
      })
    }
    ...
    insights.push({
      ...
      impact: 'medium',
      confidence: 0.75,
      actionItems: [{ action: 'Schedule preventive maintenance', impact: 'Reduce unexpected failures by 40%' }],
    })
