...
export function AllocationForm({ assetId, assetName, onClose, onSuccess }: AllocationFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.toUserId || !formData.startDate) {
      return
    }
      ...
      <div className="bg-card border border-border rounded-xl max-w-lg w-full">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              type="submit"
              disabled={createAllocation.isLoading || !formData.toUserId || !formData.startDate}
            ...
            >
              {createAllocation.isLoading ? 'Creating...' : 'Request Allocation'}
            </button>
          </div>
        </form>
      </div>
